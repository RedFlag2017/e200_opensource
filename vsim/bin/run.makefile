...
SIM_TOOL      := vcs -full64 -cpp g++-4.8 -cc gcc-4.8 -LDFLAGS -Wl,-no-as-needed 
...
SIM_EXEC      := ${RUN_DIR}/simv # ${RUN_DIR}/vvp.exec -none # The free vvp is tooooo slow to run, so just comment it out, and replaced with the fake way below
...
#	${WAV_TOOL} ${WAV_OPTIONS} & 
	cd ${RUN_DIR};	verdi -simflow -simBin simv -ssf ${TESTNAME}/wave.fsdb & 
